      ...
      <div class="box">
        ...
        <div class="pure-g" v-if="users">
          ...
          >
            <carousel :perPage="1">
              <el-tooltip
                v-if="!u.user.lastconnnection"
                class="item"
                effect="dark"
                :content="u.user.isOnline"
                placement="left"
              >
              ...
              <el-tooltip
                ...
                class="item"
                effect="dark"
                :content="'offline: '+u.user.lastconnnection"
                placement="left"
              >
                <span :class="u.user.isOnline"></span>
              </el-tooltip>
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    ...
    users.forEach(user => {
      this.sockets.unsubscribe("isOnline" + user.user.user_name);
    });
    next();
  },
  mounted() {
  ...
  methods: {
    onlinesocket() {
      ...
      users.forEach(user => {
        ...
        this.sockets.subscribe("isOnline" + user.user.user_name, data => {
          if (data.online) {
            user.user.lastconnnection = "";
            user.user.isOnline = "online";
          } else {
            user.user.isOnline = "offline";
            user.user.lastconnnection = moment(data.last).format(
              "YYYY-MM-DD HH:mm:ss"
            );
          }
        });
